...
export function getPoolLink(token0Address, token1Address = null, remove = false) {
  if (!token1Address) {
    return (
      process.env.REACT_APP_GRAVIS_SWAP_URL +
      (remove ? `/remove` : `/add`) +
...
export function getSwapLink(token0Address, token1Address = null) {
  if (!token1Address) {
    return `${process.env.REACT_APP_GRAVIS_SWAP_URL}/swap?inputCurrency=${token0Address}`
  } else {
    return `${process.env.REACT_APP_GRAVIS_SWAP_URL}/swap?inputCurrency=${
      token0Address === '0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2' ? 'ETH' : token0Address
...
export async function getBlocksFromTimestamps(timestamps, skipCount = 500) {
  ...
  // Temporary fix
  // blocks.push({ timestamp: '1619211080', number: '8203829' })
  return blocks
}
...
export const urls = {
  showTransaction: (tx) => getCurrentNetworkLinks().SCAN_LINK_TX + `${tx}/`,
  showAddress: (address) => getCurrentNetworkLinks().SCAN_LINK + `${address}/`,
  showToken: (address) => getCurrentNetworkLinks().SCAN_LINK_TOKEN + `${address}/`,
...
export function formattedPercent(percent, useBrackets = false) {
  ...
  if (percent < 0.0001 && percent > 0) {
    return (
      <Text fontWeight={500} color="#009CE1">
        {'< 0.0001%'}
      </Text>
    )
  }
  ...
  if (percent < 0 && percent > -0.0001) {
    return (
      <Text fontWeight={500} color="#FFA100">
        {'< 0.0001%'}
      </Text>
    )
  }
  ...
  if (fixedPercent > 0) {
    if (fixedPercent > 100) {
      return <Text fontWeight={500} color="#009CE1">{`+${percent?.toFixed(0).toLocaleString()}%`}</Text>
    } else {
      return <Text fontWeight={500} color="#009CE1">{`+${fixedPercent}%`}</Text>
    }
  } else {
    return <Text fontWeight={500} color="#FFA100">{`${fixedPercent}%`}</Text>
  }
}
