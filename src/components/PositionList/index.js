...
const DashGrid = styled.div`
  ...
  grid-template-areas: 'number name gravisswap return';
  ...
  @media screen and (max-width: 740px) {
    ...
    grid-template-areas: 'name gravisswap return';
  }

  @media screen and (max-width: 500px) {
    grid-template-columns: 2.5fr 1fr;
    grid-template-areas: 'name gravisswap';
  }
`
...
function PositionList({ positions }) {
        ...
        <DashGrid style={{ opacity: poolOwnership > 0 ? 1 : 0.6 }} focus={true}>
          ...
          <DataText area="name" justifyContent="flex-start" alignItems="flex-start">
            ...
            <AutoColumn gap="8px" justify="flex-start" style={{ marginLeft: '20px' }}>
              <CustomLink to={'/pair/' + position.pair.id + window.location.search}>
  ...
  const positionsSorted = useMemo(
      ...
      positions
        ...
        .sort((p0, p1) => {
          ...
          if (sortedColumn === SORT_FIELD.pancakeswap_RETURN) {
            return p0?.aliumswap?.return > p1?.aliumswap?.return ? (sortDirection ? -1 : 1) : sortDirection ? 1 : -1
          }
    ...
    [page, positions, sortDirection, sortedColumn]
  )
