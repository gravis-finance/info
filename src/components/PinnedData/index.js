      ...
      <AutoColumn gap="40px" style={{ marginTop: '2rem' }}>
        <AutoColumn gap={'12px'}>
          ...
          {Object.keys(savedPairs).filter((key) => {
              ...
              .map((address) => {
                ...
                return (
                  <RowBetween key={pair.address}>
                    <ButtonFaded onClick={() => history.push('/pair/' + address + window.location.search)}>
                      <RowFixed>
                        <TYPE.header>
                          <FormattedName
            ...
            <TYPE.light>{t('pinnedPairsWillAppear')}</TYPE.light>
          )}
        </AutoColumn>
        <ScrollableDiv gap={'12px'}>
          ...
          {Object.keys(savedTokens).filter((key) => {
              ...
              .map((address) => {
                ...
                return (
                  <RowBetween key={address}>
                    <ButtonFaded onClick={() => history.push('/token/' + address + window.location.search)}>
                      <RowFixed>
            ...
            <TYPE.light>{t('pinnedTokensWillAppear')}</TYPE.light>
          )}
        </ScrollableDiv>
      </AutoColumn>
    </RightColumn>
